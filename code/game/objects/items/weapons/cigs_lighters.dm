...
/obj/item/weapon/match
	...
	var/lit = 0
	var/smoketime = 5
	var/sound_lit = 'sound/f13items/matchstick_lit.ogg'
	var/sound_hit = 'sound/f13items/matchstick_hit.ogg'
	hitsound = 'sound/f13items/matchstick_hit.ogg'
	w_class = 1
...
/obj/item/weapon/match/proc/matchignite(mob/living/user)
	if(lit == 0)
		if(prob(50))
			lit = 1
			playsound(src, sound_lit, 25, 1, -3)
			...
			update_icon()
			if(user)
				user.visible_message("[user] has lit [src].", "<span class='notice'>You have lit [src].</span>")
		else
			playsound(src, sound_hit, 25, 1, -3)
			if(user)
				user.visible_message("[user] has tried to light [src].", "<span class='notice'>You have tried to light [src].</span>")
...
/obj/item/weapon/lighter/greyscale
	...
	icon_state = "lighter"
	sound_toggleON = 'sound/f13items/lighter_on.ogg'
	sound_toggleOFF = 'sound/f13items/lighter_off.ogg'
...
/obj/item/weapon/lighter/attack_self(mob/living/user)
	if(user.r_hand == src || user.l_hand == src)
		if(!lit)
			lit = 1
			playsound(src, sound_toggleON, 25, 1, -3)
			update_icon()
			force = 5
			damtype = "fire"
			hitsound = 'sound/items/welder.ogg'
			...
			lit = 0
			playsound(src, sound_toggleOFF, 25, 1, -3)
			update_icon()
			hitsound = "swing_hit"
			force = 0
